    ...
    <Card className="p-4 sm:p-5">
      ...
      <div className="space-y-2.5 mb-5">
        {answers.map((ans, i) => {
          ...
          if (checked) {
            if (isThis && isCorrect) cls = 'border-emerald-500/50 bg-emerald-500/10';
            else if (isThis && !isCorrect) cls = 'border-destructive/50 bg-destructive/10';
          ...
          return (
            ...
            >
              <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 text-xs font-bold ${
                checked && isThis && isCorrect ? 'bg-emerald-500 text-white'
                : checked && isThis && !isCorrect ? 'bg-destructive text-white'
                : isThis ? 'bg-primary text-primary-foreground'
                : 'bg-secondary text-muted-foreground'
              }`}>
                {checked && isThis && isCorrect ? <CheckCircle2 className="w-4 h-4" />
                  : checked && isThis && !isCorrect ? <XCircle className="w-4 h-4" />
                  : num}
              </div>
              <span className={`text-sm sm:text-base font-medium ${checked && !isThis ? 'text-muted-foreground' : 'text-foreground'}`}>
                {ans}
              </span>
            </button>
          );
        ...
        <div className={`p-3.5 rounded-xl mb-4 ${isCorrect ? 'bg-emerald-500/10 border border-emerald-500/30' : 'bg-destructive/10 border border-destructive/30'}`}>
          <div className="flex items-center gap-2">
            ...
            <span className={`font-bold text-sm ${isCorrect ? 'text-emerald-400' : 'text-destructive'}`}>
              {isCorrect ? 'נכון מאוד! 🎉' : 'תשובה שגויה — התשובה הנכונה תוצג בסוף המבחן'}
            </span>
          </div>
        </div>
      )}
